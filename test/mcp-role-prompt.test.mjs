...
test("initializes generically, then exposes one verified role prompt on dynamic MCP surfaces", async () => {
  ...
  try {
    ...
    assert.equal(publicResult.contract_version, "1.2.0");
    ...
    assert.match(guidance, /Contract: pilink-collaboration\/implementer@1\.2\.0/);
    assert.match(guidance, /PILINK SHARED COLLABORATION CONTRACT v1\.2\.0/);
    assert.match(guidance, /PILINK IMPLEMENTER ROLE v1\.2\.0/);
