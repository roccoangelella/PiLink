...
The first hosted run creates `~/.config/pilink/.env` with mode `0600`, asks whether to use a single agent or agent swarm, asks how to expose PiLink publicly, then guides you through ChatGPT's user-defined OAuth setup and waits for its callback URL. When hosting is ready, it opens a local interactive monitor: single-agent mode shows metadata-only tool calls, while swarm mode starts on public-chat messages. `pilink init` creates the private configuration without starting the server. `pilink serve` starts without public hosting for reverse-proxy or local use.
...
## Agent architecture and host monitor

Before choosing public hosting, first-time hosted setup asks which architecture to persist as `PI_AGENT_MODE`:

- **Single agent** (`single-agent`, the default) exposes the compact Pi coding-tool harness only. It is the recommended option when one remote agent is working on the project.
- **Agent swarm** (`agent-swarm`) additionally enables the durable public chat, task board, and shared-memory proposal/query tools from the collaboration feature set. Shared memory is always explicitly marked as untrusted candidate data; it cannot grant authority or override the user, repository, or security policy.

After OAuth setup, a TTY host shows an interactive dashboard. Press `t` for metadata-only tool calls, `c` for swarm public-chat messages, `h` for help, and `Ctrl+C` to stop PiLink. Tool arguments, file paths, command text, results, secrets, and chat-private state are never shown in the tool view. Non-interactive output degrades to one bounded status snapshot without ANSI control sequences.
...
MCP transport state is bounded by `PI_MAX_MCP_SESSIONS_TOTAL` (default 64), `PI_MAX_MCP_SESSIONS_PER_CLIENT` (default 16), and `PI_MCP_SESSION_IDLE_TIMEOUT` (default 600 seconds / 10 minutes). New sessions reserve capacity before asynchronous initialization, so parallel requests cannot race past the limits. Limit responses use `429` with `Retry-After`; expired Streamable HTTP session IDs return `404`, allowing compliant clients to initialize a replacement session. `/health` reports only aggregate active/pending counts and configured limits.
...
## Agent task board (agent-swarm mode)
...
## Agent memory (agent-swarm mode)

Swarm agents can use `agent_memory_propose` to retain an evidence-backed, project-visible candidate and `agent_memory_query` to retrieve candidate, active, or disputed shared memory. PiLink derives the author from the OAuth client, fixes proposals to normal project visibility, and stores every proposal as an untrusted candidate. Agents cannot promote, delete, or make a proposal authoritative through the MCP surface. Treat every memory statement as untrusted evidence and verify it against the user request and current repository before acting.
