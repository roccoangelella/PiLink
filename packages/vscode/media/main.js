(function () {
  ...
  const EMPTY_STATE = {
    runtimeMode: { mode: "single", configured: false },
    configured: null,
    trusted: null,
    workspace: "",
    configPath: "",
  ...
  let uiMode = restoredSurfaceMode(restoredUiState);

  function restoredSurfaceMode(value) {
    if (!isRecord(value)) return "remote";
    // Version 2 stored the execution surface as `mode`. Keep that value as
    // the surface migration; it must never become the server runtime mode.
    if ((value.version === 2 || value.version === UI_STATE_VERSION) && value.mode === "local") return "local";
    return "remote";
  }
  ...
  function normalizeState(value) {
    ...
    const runtimeMode = isRecord(source.runtimeMode) ? source.runtimeMode : {};
  ...
  function renderRuntimeModeChooser() {
    const configured = currentState.runtimeMode.configured;
    const section = el("section", "runtime-mode-picker" + (configured ? " runtime-mode-picker--configured" : ""));
    section.setAttribute("aria-labelledby", "vspilink-runtime-mode-title");
    ...
    copy.querySelector("h2").id = "vspilink-runtime-mode-title";
    const choices = el("div", "runtime-mode-picker__choices");
    choices.appendChild(runtimeModeChoice(
      "Single-agent",
      "Classic Pi chat",
      "One supervised Pi agent works in this workspace. No public orchestration state is enabled.",
      "single",
      "single",
    ));
    choices.appendChild(runtimeModeChoice(
      "Public chat & orchestration",
      "ChatGPT MCP coordination",
      "Authenticated ChatGPT MCP clients can use collaboration tools, shared chat, tasks, and agent supervision. Hosting and OAuth remain separate.",
      "collaboration",
      "collaboration",
    ));
    append(section, copy, choices);
    return section;
  }

  function runtimeModeChoice(title, subtitle, description, value, mode) {
    const card = el("article", "runtime-mode-choice" + (currentState.runtimeMode.mode === mode && currentState.runtimeMode.configured ? " is-selected" : ""));
    const selected = currentState.runtimeMode.mode === mode && currentState.runtimeMode.configured;
    const action = makeButton(selected ? title + " · selected" : "Use " + title, "selectRuntimeMode", {
      variant: selected ? "ghost" : mode === "collaboration" ? "primary" : "secondary",
      compact: true,
      value: value,
      disabled: selected || currentState.trusted === false,
      className: "runtime-mode-choice__button",
    });
    append(card,
      el("h3", "runtime-mode-choice__title", title),
      el("p", "runtime-mode-choice__subtitle", subtitle),
      el("p", "runtime-mode-choice__description", description),
      action,
    );
  ...
  function renderRuntimeModePrompt() {
  ...
  function render() {
    ...
    fragment.appendChild(renderRuntimeModeChooser());
    if (currentState.runtimeMode.configured) {
      if (uiMode === "remote") {
        fragment.appendChild(renderChatGptWorkspace());
        if (currentState.runtimeMode.mode === "collaboration") {
      ...
      fragment.appendChild(renderRuntimeModePrompt());
    }
  ...
  function renderChatGptWorkspace() {
    ...
    if (currentState.runtimeMode.mode === "single") {
      shell.appendChild(renderRemoteEmpty(
        "Single-agent workflow selected",
        "ChatGPT MCP remains a separate execution surface for this workspace, but shared public chat, tasks, and orchestration are disabled until you choose the collaboration workflow above.",
        { label: "Use collaboration workflow", command: "selectRuntimeMode", value: "collaboration" },
      ));
    }
    ...
    append(actions,
      ...
      makeButton("Open collaboration monitor", "openCollaborationMonitor", {
        variant: "secondary",
        compact: true,
        icon: ">_",
        disabled: currentState.runtimeMode.mode !== "collaboration",
      })
    );
  ...
  function renderChatGptConnectionGuide() {
    ...
    append(endpointActions,
      ...
      makeButton("Agent monitor", "openCollaborationMonitor", {
        variant: "ghost",
        compact: true,
        icon: ">_",
        disabled: currentState.runtimeMode.mode !== "collaboration",
  ...
  function renderLocalModeIntro() {
    ...
    append(copy,
      el("p", "local-mode-intro__eyebrow", "PI LOCAL SURFACE"),
      el("h2", "local-mode-intro__title", "Pi Local chat · single-agent surface"),
      el("p", "local-mode-intro__description", "This is the classic single-agent conversation surface. It uses the provider and model configured in Pi; it does not connect ChatGPT or expose public orchestration by itself.")
    );
  ...
  function renderCompactAgents() {
    ...
    const atAgentLimit = currentState.agentRuntime.maxConcurrent > 0 &&
      currentState.agentRuntime.active >= currentState.agentRuntime.maxConcurrent;
    append(actions,
      makeButton("New agent", "spawnAgent", { variant: "secondary", compact: true, icon: "+", disabled: !isChatReady() || atAgentLimit }),
  ...
  function renderServerDetails() {
    ...
    [
      ...
      { label: "Agent monitor", command: "openCollaborationMonitor", icon: "◎", disabled: currentState.runtimeMode.mode !== "collaboration" },
  ...
  function updateComposerState() {
    const ready = currentState.runtimeMode.configured && uiMode === "local" && isChatReady();
