...
export const COLLABORATION_SHARED_CONTRACT_VERSION = "1.2.0" as const;
...
3. Otherwise, when agent_task_claim_next is available, use it to atomically select and claim the highest-priority ready task compatible with the verified role, dependencies, authorization, and non-overlapping scope. Do not independently rank the queue in model reasoning. Use a direct task claim only for an explicitly selected task ID or lease renewal.
...
const MANAGER_PROMPT_FRAGMENT = `PILINK MANAGER ROLE v1.2.0
...
const RESEARCHER_PROMPT_FRAGMENT = `PILINK RESEARCHER ROLE v1.2.0
...
const IMPLEMENTER_PROMPT_FRAGMENT = `PILINK IMPLEMENTER ROLE v1.2.0
...
Implement the smallest coherent change that satisfies acceptance criteria. Add or update focused tests, inspect the diff, and provide the integration or full-suite evidence required by task risk. Prefer bounded workspace/fixed execution profiles for verification when available, and use unrestricted shell only when broader authority is actually necessary. Never claim success from prose approval alone.
...
const AI_ENGINEER_PROMPT_FRAGMENT = `PILINK AI ENGINEER ROLE v1.2.0
...
const COLLABORATOR_PROMPT_FRAGMENT = `PILINK COLLABORATOR FALLBACK ROLE v1.2.0
