...
export async function openAutomaticPortMappings(): Promise<ManagedPortMappings> {
  ...
  try {
    ...
    for await (const gateway of upnp.findGateways({ signal: AbortSignal.timeout(8_000) })) {
      try {
        return await mapPorts(gateway, localHost);
      } catch (error) {
  ...
  try {
    return await mapPorts(pmpNat(route.gateway, { ttl: MAPPING_TTL_MS, autoRefresh: true, description: MAPPING_DESCRIPTION }), localHost);
  } catch (error) {
    if (error instanceof DirectNetworkError && !error.canUseManualFallback) throw error;
...
export async function mapPorts(gateway: Gateway, localHost: string): Promise<ManagedPortMappings> {
  try {
    ...
    const publicIp = await gateway.externalIp();
