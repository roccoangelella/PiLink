...
const interactiveMonitoring = process.env.PI_INTERACTIVE_MONITOR === "true";
const swarmMode = config.agentMode === "agent-swarm";

function protocolLog(...args: unknown[]): void {
  if (!interactiveMonitoring) console.error(...args);
}
...
// ── Request logging ──────────────────────────────────────────
// Hosted runs use the local interactive monitor instead of noisy HTTP/MCP logs.
if (!interactiveMonitoring) {
  ...
  if (totalExceeded || clientExceeded) {
    ...
    res.status(429).json({
      ...
      error_description: clientExceeded
        ? "OAuth client has reached its MCP session limit"
        : "PiLink has reached its total MCP session limit",
      limits: {
...
async function closeTransportForClientLifecycle(sessionId: string, managed: ManagedTransport): Promise<void> {
  await closeManagedTransport(sessionId, managed, "OAuth");
}
...
async function sweepIdleMcpSessions(): Promise<void> {
  const now = Date.now();
  await Promise.all(Object.entries(transports).map(async ([sessionId, managed]) => {
    if (managed.inFlightRequests > 0) return;
    if (now - managed.lastActivityAtMs < idleSessionTimeoutMs) return;
    protocolLog(
      `[MCP] Expiring idle session ${sessionId} for client ${managed.clientId} ` +
      `after ${config.mcpSessionIdleTimeoutSeconds}s without activity.`,
    );
...
app.post("/sse", authenticateBearer, async (req, res) => {
  ...
  try {
    ...
    protocolLog("[MCP] New Streamable HTTP session initializing...");
    ...
    const handle = createMcpServer(
      ...
      swarmMode ? getAgentChatBroker() : undefined,
      getToolAuditLog(),
      undefined,
      swarmMode ? getAgentTaskStore() : undefined,
      swarmMode ? getAgentMemoryStore() : undefined,
    );
    ...
    const transport = new StreamableHTTPServerTransport({
      ...
      onsessioninitialized: (sid) => {
          protocolLog(`[MCP] Streamable HTTP session created: ${sid}`);
        if (managed) {
    ...
    const cleanup = () => {
      ...
      if (sid && transports[sid]) {
          protocolLog(`[MCP] Streamable HTTP session closed: ${sid}`);
...
app.get("/sse", authenticateBearer, async (req, res) => {
  ...
  if (sessionId) {
    ...
    if (transport instanceof StreamableHTTPServerTransport) {
      touchTransport(managed);
      protocolLog(`[MCP] Streamable HTTP SSE stream opened for session: ${sessionId}`);
      try {
  ...
  protocolLog("[MCP] Legacy SSE session starting...");
  const sessionClient = resolveNewSessionClient(req, res);
  if (!sessionClient) return;
  const releaseReservation = reserveSessionSlot(sessionClient.clientId, res);
  if (!releaseReservation) return;
  ...
  releaseReservation();
  protocolLog(`[MCP] Legacy SSE session created: ${transport.sessionId}`);

  res.on("close", () => {
    protocolLog(`[MCP] Legacy SSE session closed: ${transport.sessionId}`);
...
app.delete("/sse", authenticateBearer, async (req, res) => {
  ...
  if (sessionId && transports[sessionId]) {
    ...
    const transport = managed.transport;
    if (transport instanceof StreamableHTTPServerTransport) {
      protocolLog(`[MCP] Streamable HTTP session deleted: ${sessionId}`);
      try {
