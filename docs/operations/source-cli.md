...
npm run cli -- install-vscode-plugin
```

Launcher experiences are presented in order: 1 Single agent (`--mode single`), 2 Agents chat (`--mode collaboration`), and 3 CLI pilink-endpoint (`--mode cli` / `pilink gateway start`; gateway subcommands remain). PiLink for VS Code is installed separately with `pilink install-vscode-plugin`. Server runtime capability modes (`single` and `collaboration`) remain strictly distinct from launch experiences.
