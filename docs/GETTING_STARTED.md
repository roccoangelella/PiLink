...
PiLink limits active transport state across Streamable HTTP and legacy SSE. `PI_MAX_MCP_SESSIONS_TOTAL` defaults to 64, `PI_MAX_MCP_SESSIONS_PER_CLIENT` defaults to 8, and `PI_MCP_SESSION_IDLE_TIMEOUT` defaults to 3,600 seconds. Initializations reserve a slot before asynchronous MCP setup. Requests beyond a limit receive `429 Too Many Requests` and `Retry-After: 5`; expired Streamable HTTP session IDs receive `404`, which tells compliant clients to create a new session. Aggregate usage and limits are visible on `/health`.
