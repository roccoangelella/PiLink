...
## 3. Choose agent architecture and public hosting on first start

The first hosted `pilink start` first asks which agent architecture to save in the private configuration:

1. **Single agent** (`single-agent`) keeps the compact coding-tool harness and is recommended when one remote agent will use the project.
2. **Agent swarm** (`agent-swarm`) enables durable public chat, the task board, and governed shared-memory proposals/query for multiple authorized agents. Shared memory remains explicitly untrusted candidate data, never policy or authority.

It then asks which public-hosting mode to save:
...
### Agent chat coordination (agent-swarm mode)
...
- After OAuth setup, hosted TTYs display a local interactive monitor rather than HTTP/MCP request logs. Press `t` for metadata-only tool calls, `c` for public chat in swarm mode, `h` for help, and `Ctrl+C` to stop PiLink. The tool view deliberately excludes arguments, file paths, command text, results, and secrets.
