...
VSPiLink derives from
[roccoangelella/PiLink](https://github.com/roccoangelella/PiLink) and preserves
two upstream development lines that have diverged:
...
VSPiLink adds a VS Code product layer without treating one upstream branch as a
...
| Feature integration baseline | `b629c0ee004b7e792125158879c55ee00bd89310` | Final upstream files imported where non-overlapping; runtime, Textual, tests, and protocols adapted where VSPiLink has additive changes |
...
VSPiLink completed the selective integration of those fixes. The exact final upstream
...
Textual monitor, and their tests were adapted without discarding VSPiLink's
...
VSPiLink keeps three authentication domains separate:

1. **ChatGPT plugin → VSPiLink MCP OAuth**, including DCR and the manual
...
evidence, not a VSPiLink claim of authorship or complete integration.
...
4. preserve VSPiLink additions without renaming upstream public tools;
5. update or migrate private state explicitly;
6. run original upstream tests plus VSPiLink integration tests;
