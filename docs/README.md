...
| [`COLLABORATION_MEMORY_ARCHITECTURE.md`](COLLABORATION_MEMORY_ARCHITECTURE.md) | Accepted architecture/decision and research basis | Current accepted target for governed agent memory: structured private store, typed memory classes, file projections, progressive retrieval, provenance, supersession, deletion, and evaluation. No runtime implementation is claimed. | Manager and memory architecture maintainer | Accepted in commit `8337a8f`. Refines the memory section of `COLLABORATION_MANAGER_DECISIONS.md` and supersedes memory-specific recommendations in earlier drafts where they conflict. |
...
| [`SCHEDULER_OWNERSHIP_ACCEPTANCE_REVIEW.md`](SCHEDULER_OWNERSHIP_ACCEPTANCE_REVIEW.md) | Acceptance review, currently untracked | Exact review of task-authority commit `3847741` and scheduler commit `9883ca1`. Task authority is accepted; scheduler acceptance is conditional pending the listed repair and rereview. | Independent reviewer | Do not treat its proposed repairs as implemented until a new exact commit verdict exists. Untracked status means it is not yet an accepted repository artifact. |
| [`AGENT_ACTIVITY_DESIGN.md`](AGENT_ACTIVITY_DESIGN.md) | Draft, currently untracked | Early activity-ledger design proposal. Useful historical rationale, but not the current runtime contract. | Original design author; manager decides promotion or archival | The implemented activity core `82f016d`, manager decisions, and acceptance review supersede it wherever they are more specific. Do not promote this file merely by linking it here. |
| [`ENFORCED_COLLABORATION_DESIGN.md`](ENFORCED_COLLABORATION_DESIGN.md) | Draft, currently untracked | Broad proposal for persistent roles, governed memory, standups, review rounds, and permits. It explicitly says not to implement until review converges. | Original design author; manager decides promotion or archival | Partially superseded by `COLLABORATION_PROTOCOL_RESEARCH_REVIEW.md`, `COLLABORATION_MANAGER_DECISIONS.md`, `COLLABORATION_ROLE_CONTRACTS.md`, and `COLLABORATION_MEMORY_ARCHITECTURE.md`. Read only for historical ideas not already decided elsewhere. |
...
At the time of this review, `AGENT_ACTIVITY_DESIGN.md`, `ENFORCED_COLLABORATION_DESIGN.md`, and `SCHEDULER_OWNERSHIP_ACCEPTANCE_REVIEW.md` are untracked. Listing them here is inventory only and does not promote them. `docs/GETTING_STARTED.md` is deleted in the working tree and is therefore not a current documentation target; do not add new links to it unless the manager restores or replaces it.
...
- `COLLABORATION_MEMORY_ARCHITECTURE.md` makes the memory portions precise and supersedes conflicting memory details in drafts.
...
4. The accepted subsystem document relevant to the current decision:
   - [`COLLABORATION_MEMORY_ARCHITECTURE.md`](COLLABORATION_MEMORY_ARCHITECTURE.md)
